...
function letternumber(e, str)
		...
		function compare_report()
		{
				...
				document.entryfrm.action="reports/im_hourly_based_flight_clearance01.jsp?icp="+document.entryfrm.compare_icp.value;
				//document.entryfrm.action="test2.jsp?&icp="+document.entryfrm.compare_icp.value;
				document.entryfrm.submit();
				return true;
		}

		function vishwa_special()
		{
				document.entryfrm.target="_self";
				document.entryfrm.action="reports/im_hourly_based_flight_clearance01.jsp?filter_icp="+document.entryfrm.compare_icp.value;
				//document.entryfrm.action="test2.jsp?&icp="+document.entryfrm.compare_icp.value;
				document.entryfrm.submit();
				return true;
		}
		...
		{
				document.entryfrm.target="_self";
				document.entryfrm.action="im_icp_dashboard_00_test_validation.jsp?&icp="+document.entryfrm.compare_icp.value+"&default_hrs="+document.entryfrm.default_hrs.value;
...
<!-- 			<option value="All" <%if(filter_icp.equals("All")){%> selected<%}%>>All ICPs</option>
 --><%
			...
			while(rsTemp.next())
			{	
				String currentIcpSrno= rsTemp.getString("ICP_SRNO");
				
				if( (ipToIcpMapping.get(Client_IP).split("#").length>=4) || (ipToIcpMapping.containsKey(Client_IP) && ipToIcpMapping.get(Client_IP).contains(currentIcpSrno)))
				{
					%>
						<option value="<%=rsTemp.getString("ICP_SRNO")%>" <%if(filter_icp.equals(rsTemp.getString("ICP_SRNO"))){%> selected<%}%>><%=rsTemp.getString("ICP_DESC")%></option>
					<%
...
%> 
			...
			&nbsp;&nbsp;<button class="btn btn-primary btn-sm" type="button" onClick="vishwa_special();"> Generate_ICP_SPecial </button>
...
<%////////////////////20/11/2023//////////////////////////
	...
	Map<String, String> arr_Daywise = new HashMap<String,String>(); // For storing arr_Daywise .

	Map<String, String> dep_Daywise = new HashMap<String,String>(); // For storing dep_Daywise .
	Map<String, String> total_Daywise = new HashMap<String,String>(); // For storing total_Daywise .
		...
		<% 
			...
			for (int i = 0; i < weekListPAX.length; i++) {
				...
				t_Total = Integer.parseInt(weeklyArrPAX[i]) + Integer.parseInt(weeklyDepPAX[i]);
					arr_Daywise.put(weekListPAX[i],weeklyArrPAX[i]);
					dep_Daywise.put(weekListPAX[i],weeklyDepPAX[i]);
					total_Daywise.put(weekListPAX[i],(weeklyArrPAX[i] + weeklyDepPAX[i]));
...
try
	{
	SQLQUERY_Arr = "select SUM(HOURLY_INDIAN_COUNT) as sum_hourly_indian_count, SUM(HOURLY_FOREIGNER_COUNT) as sum_hourly_foreigner_count,icp_description,to_char(pax_boarding_date,'Mon-dd') as pax_boarding_date_2, pax_boarding_date,ICP_SRNO,sum(hourly_evisa_count) as sum_evisa_count, sum(hourly_voa_count) as sum_hourly_voa_count, sum(hourly_regular_visa_count) as hourly_regular_visa_count, sum(hourly_visa_exempted_count),sum(hourly_oci_count) as sum_hourly_oci_count,sum(hourly_foreigner_count), table_type from  imigration.IM_DASHBOARD_COMBINED_PAX_BOARDING_DATE where ICP_SRNO = '"+ filter_icp +"' and  pax_boarding_date >= trunc(sysdate-6) and pax_boarding_date <= trunc(sysdate)  and table_type='IM_TRANS_ARR_TOTAL'  group by pax_boarding_date,table_type,icp_description,ICP_SRNO order by pax_boarding_date";
	...
	while (rsTemp.next()){
		...
		foreign_Precent_Arr = 0;
			...
			pForeigner_Arr.append( Math.round(foreign_Precent_Arr) + ",");
					arr_Indian_Daywise.put(rsTemp.getString("pax_boarding_date_2"),rsTemp.getInt("sum_hourly_indian_count"));
					arr_Foreigner_Daywise.put(rsTemp.getString("pax_boarding_date_2"),rsTemp.getInt("sum_hourly_foreigner_count"));
...
try
	{
	SQLQUERY = "select SUM(HOURLY_INDIAN_COUNT) as sum_hourly_indian_count, SUM(HOURLY_FOREIGNER_COUNT) as sum_hourly_foreigner_count,icp_description,to_char(pax_boarding_date,'Mon-dd') as pax_boarding_date_2, pax_boarding_date,ICP_SRNO,sum(hourly_evisa_count) as sum_evisa_count, sum(hourly_voa_count) as sum_hourly_voa_count, sum(hourly_regular_visa_count) as hourly_regular_visa_count, sum(hourly_visa_exempted_count),sum(hourly_oci_count) as sum_hourly_oci_count,sum(hourly_foreigner_count), table_type from  imigration.IM_DASHBOARD_COMBINED_PAX_BOARDING_DATE where ICP_SRNO = '"+ filter_icp +"' and  pax_boarding_date >= trunc(sysdate-6) and pax_boarding_date <= trunc(sysdate)  and table_type='IM_TRANS_DEP_TOTAL'  group by pax_boarding_date,table_type,icp_description,ICP_SRNO order by pax_boarding_date";
	...
	while (rsTemp.next()){
		...
		foreign_Precent = 0;
			...
			dep_Indian_Daywise.put(rsTemp.getString("pax_boarding_date_2"),rsTemp.getInt("sum_hourly_indian_count"));
			dep_Foreigner_Daywise.put(rsTemp.getString("pax_boarding_date_2"),rsTemp.getInt("sum_hourly_foreigner_count"));
			%>
			<tr style="font-size: 14px; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
...
<%///////////////// Indian/Foreigner Count ///////////////////%>
<%//////////////////////	Arrival : Indian/Foreigner Statistics in last 7 days - Start	/////////////////////////////////
	String WeeklyNationality_Arr_Val = "";
	String weeklyNationalityXAxis_Arr_Val = "";
	int weekelyIndianCount_Arr_Val = 0;
	int weekelyForeignerCount_Arr_Val = 0;
	String SQLQUERY_Arr_Val = "";


	StringBuilder weekDaysNationality_Arr_Val = new StringBuilder();
	StringBuilder weekIndian_Arr_Val = new StringBuilder();
	StringBuilder weekForeigner_Arr_Val = new StringBuilder();
	StringBuilder pIndian_Arr_Val = new StringBuilder();
	StringBuilder pForeigner_Arr_Val = new StringBuilder();

	String strweekDaysNationality_Arr_Val = "";
	String strweekIndian_Arr_Val = "";
	String strweekForeigner_Arr_Val = "";
%>
<div class="container-fluid">
	<div class="row">
	<%/////////////////////Table for Data comparision////////////////////////%>
<%
				//SQLQUERY_Arr_Val = "select PAX_BOARDING_DATE, COUNT(1) AS c_count FROM IM_TRANS_ARR_TOTAL WHERE PAX_BOARDING_DATE >= trunc(sysdate-8) AND ICP_SRNO = '"+ filter_icp +"' and  PAX_NALTY_CODE <> 'IND' group by pax_boarding_date";
...
///////////////// Indian/Foreigner Count ///////////////////%>
	...
	try {
		WeeklyVisaQuery = "select icp_description,to_char(pax_boarding_date,'Mon-dd') as pax_boarding_date_2, pax_boarding_date,ICP_SRNO,sum(hourly_evisa_count) as sum_evisa_count, sum(hourly_voa_count) as sum_hourly_voa_count, sum(hourly_regular_visa_count) as hourly_regular_visa_count, sum(hourly_visa_exempted_count) as sum_hourly_visa_exempted_count,sum(hourly_oci_count) as sum_hourly_oci_count,sum(hourly_foreigner_count), table_type from  imigration.IM_DASHBOARD_COMBINED_PAX_BOARDING_DATE where ICP_SRNO = '" + filter_icp + "' and pax_boarding_date >= trunc(sysdate-6) and pax_boarding_date <= trunc(sysdate) and table_type='IM_TRANS_ARR_TOTAL'  group by pax_boarding_date,table_type,icp_description,ICP_SRNO order by pax_boarding_date";
...
<%////////////////	Table - Arrival : Visa Clearance in last 7 days - Start	///////////////////////%>
		...
		<div class="col-sm-3" style="flex:2;">
	<table class="tableDesign">
			<tr style="font-size: 14px;  text-align: right; color:white; border-color: #003a6d;height:40px;">
				...
				<th style="text-align: center;background-color:#ed3e12;border-color: #b8300e;width:20%; text-align: right;">Difference&nbsp;&nbsp;</th>
			</tr>
		<% 
			...
			%>
			<tr style="font-size: 14px; font-family: 'Arial', serif; text-align: center;height:20px;">
				<td style="background-color:#f69a83;border-color: #ed3e12;width:20%; font-weight: bold;text-align: center;"><%=weekListWeekly[i].equals("0") ? "&nbsp;" : weekListWeekly[i].replace("-","&nbsp;")%></td>
				...
				<td style="background-color:#fef6f4;border-color: #ed3e12;width:20%; font-weight: bold; text-align: right;"><%=ExWeekly[i].equals("0") ? "&nbsp;" : getIndianFormat(Integer.parseInt(ExWeekly[i]))%>&nbsp;&nbsp;
				</td>
				<td style="font-size: 13px;background-color:#fce3dd;border-color: #ed3e12;width:20%; font-weight: bold; text-align: right;"><%=arr_Foreigner_Daywise.get(weekListWeekly[i])%>&nbsp;-&nbsp;<%=((Integer.parseInt(eVisaWeekly[i])) + (Integer.parseInt(OCIWeekly[i])) + (Integer.parseInt(RegularWeekly[i])) + (Integer.parseInt(VOAWeekly[i])) + (Integer.parseInt(ExWeekly[i])))%>&nbsp;=&nbsp;<%=(arr_Foreigner_Daywise.get(weekListWeekly[i]) - ((Integer.parseInt(eVisaWeekly[i])) + (Integer.parseInt(OCIWeekly[i])) + (Integer.parseInt(RegularWeekly[i])) + (Integer.parseInt(VOAWeekly[i])) + (Integer.parseInt(ExWeekly[i]))))%>
				</td>
			</tr>
<%	}
			%>
...
<div class="col-sm-8" style="flex:4;">
...
int weekelyFailure_Arr = 0;
int weekelyMisc_Arr = 0;
...
StringBuilder weekFailure_Arr = new StringBuilder();
StringBuilder weekMisc_Arr = new StringBuilder();
	...
	<div class="col-sm-3" style="flex:6;">
		<table class="tableDesign" >
			<tr style="font-size: 14px; font-family: 'Arial', serif;color: white; font-weight: bold; text-align: center;border-color: #1192e8;height:40px;">
				<th colspan="7" style="text-align: center;background-color:#007d79;border-color: #007d79;width:20%; text-align: center;">Arrival : Biometric Enrollment/Verification/Exemption Statistics in last 7 days</th>
			</tr>
			<tr style="font-size: 14px; font-family: 'Arial', serif;color: white; font-weight: bold; text-align: center;border-color: #1192e8;height:40px;">
				...
				<th style="text-align: center;background-color:#007d79;border-color: #004a48;width:20%; text-align: right;">Failure&nbsp;&nbsp;</th>
				<th style="text-align: center;background-color:#007d79;border-color: #004a48;width:20%; text-align: right;">Misc&nbsp;&nbsp;</th>
				<th style="text-align: center;background-color:#007d79;border-color: #004a48;width:20%; text-align: right;">Diff&nbsp;&nbsp;</th>
			</tr>
<%
try
	{
	SQLQUERY_B_Arr = "select icp_description,to_char(pax_boarding_date,'Mon-dd') as pax_boarding_date_3, pax_boarding_date,ICP_SRNO,sum(HOURLY_BIO_ENROLLED) as sum_HOURLY_BIO_ENROLLED, sum(HOURLY_BIO_VERIFIED) as sum_HOURLY_BIO_VERIFIED, sum(HOURLY_BIO_EXEMPTED) as sum_HOURLY_BIO_EXEMPTED, sum(HOURLY_BIO_FAILURE) as sum_HOURLY_BIO_FAILURE,sum(HOURLY_BIO_MISC) as sum_HOURLY_BIO_MISC, table_type from imigration.IM_DASHBOARD_COMBINED_PAX_BOARDING_DATE where ICP_SRNO = '" + filter_icp + "' and pax_boarding_date >= trunc(sysdate-6) and pax_boarding_date <= trunc(sysdate) and table_type='IM_TRANS_ARR_TOTAL'  group by pax_boarding_date,table_type,icp_description,ICP_SRNO order by pax_boarding_date";
	...
	while (rsTemp.next()){
		...
		Failure_Precent_Arr = 0;
			...
			weekelyFailure_Arr = rsTemp.getInt("sum_HOURLY_BIO_FAILURE"); 
			weekelyMisc_Arr = rsTemp.getInt("sum_HOURLY_BIO_MISC"); 
			...
			weekFailure_Arr.append(weekelyFailure_Arr +",");
			weekMisc_Arr.append(weekelyMisc_Arr +",");
			...
			Failure_sum_hunder_Arr = weekelyFailure_Arr * 100;
			Misc_sum_hunder_Arr = weekelyMisc_Arr * 100;
			Bio_Total_sum_number_Arr = weekelyEnroll_Arr + weekelyVerified_Arr + weekelyExempted_Arr + weekelyFailure_Arr + weekelyMisc_Arr;
			...
			Failure_Precent_Arr = (double) Failure_sum_hunder_Arr / Bio_Total_sum_number_Arr;
			Misc_Precent_Arr = (double) Misc_sum_hunder_Arr / Bio_Total_sum_number_Arr;
			...
			B_Failure_Arr.append( Math.round(Failure_Precent_Arr) + ",");
			B_Misc_Arr.append( Math.round(Misc_Precent_Arr) + ",");
			%>
			<tr style="font-size: 14px; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
				...
				<td style="background-color:#f0ffff;border-color:#007977;width:33.33%; font-weight: bold; text-align: right;"><%=rsTemp.getInt("sum_HOURLY_BIO_FAILURE") == 0 ? "&nbsp;": getIndianFormat(rsTemp.getInt("sum_HOURLY_BIO_FAILURE"))%>&nbsp;</td>
				<td style="background-color:#f0ffff;border-color:#007977;width:33.33%; font-weight: bold; text-align: right;"><%=rsTemp.getInt("sum_HOURLY_BIO_MISC") == 0 ? "&nbsp;": getIndianFormat(rsTemp.getInt("sum_HOURLY_BIO_MISC"))%>&nbsp;</td>
				<td style="background-color:#f0ffff;border-color:#007977;width:33.33%; font-weight: bold; text-align: right;font-size:12px;"><%=getIndianFormat(arr_Foreigner_Daywise.get(rsTemp.getString("pax_boarding_date_3")))%>&nbsp;-&nbsp;<%=getIndianFormat(rsTemp.getInt("sum_HOURLY_BIO_ENROLLED") + rsTemp.getInt("sum_HOURLY_BIO_VERIFIED") + rsTemp.getInt("sum_HOURLY_BIO_EXEMPTED") + rsTemp.getInt("sum_HOURLY_BIO_FAILURE") + rsTemp.getInt("sum_HOURLY_BIO_MISC")) %><BR>=<font style="color:#007d79;"><%=getIndianFormat(arr_Foreigner_Daywise.get(rsTemp.getString("pax_boarding_date_3")) - (rsTemp.getInt("sum_HOURLY_BIO_ENROLLED") + rsTemp.getInt("sum_HOURLY_BIO_VERIFIED") + rsTemp.getInt("sum_HOURLY_BIO_EXEMPTED") + rsTemp.getInt("sum_HOURLY_BIO_FAILURE") + rsTemp.getInt("sum_HOURLY_BIO_MISC")))%><font>
				</td>
			</tr>
			<%
	}
			...
			strweekFailure_Arr = B_Failure_Arr.toString();
			strweekFailure_Arr = strweekFailure_Arr.substring(0,strweekFailure_Arr.length()-1);

			strweekMisc_Arr = B_Misc_Arr.toString();
			strweekMisc_Arr = strweekMisc_Arr.substring(0,strweekMisc_Arr.length()-1);
	}
	catch(Exception e)
	{
		e.printStackTrace();
...
<div class="col-sm-3" style="flex:3;">
<div class="card" style="border: solid 3px #007d79; border-radius: 20px;height:400px;">
<div class="card-body" style=" height:400px;">
				...
				var chartInstances = this.chart,
					...
					this.data.datasets.forEach(function (dataset, i) {
						...
						metas.data.forEach(function (bar, index) {
							...
							ctx.fillText("", bar._model.x-15, bar._model.y+6 );							
						});
					});
				}
			},
...
<%//////////////////////	Arrival : Biometric Enrollment/Verification/Exemption Statistics in last 7 days - Start	/////////////////////////////////
	...
	int weekelyFailure_Arrh = 0;
	int weekelyMisc_Arrh = 0;
	...
	StringBuilder weekFailure_Arrh = new StringBuilder();
	StringBuilder weekMisc_Arrh = new StringBuilder();
	...
	StringBuilder B_Failure_Arrh = new StringBuilder();
	StringBuilder B_Misc_Arrh = new StringBuilder();

	String strweekDaysBio_Arrh = "";
	String strweekEnroll_Arrh = "";
	String strweekVerified_Arrh = "";
	String strweekExempted_Arrh  = "";
	String strweekFailure_Arrh  = "";
	String strweekMisc_Arrh  = "";

	double Enroll_Precent_Arrh = 0;
	double Verified_Precent_Arrh = 0;
	double Exempted_Precent_Arrh = 0;
	double Failure_Precent_Arrh = 0;
	double Misc_Precent_Arrh = 0;

	int Bio_Total_sum_number_Arrh = 0;
	int Enroll_sum_hunder_Arrh = 0;
	int Verified_sum_hunder_Arrh = 0;
	int Exempted_sum_hunder_Arrh = 0;
	int Failure_sum_hunder_Arrh = 0;
	int Misc_sum_hunder_Arrh = 0;

	%>
	<div class="col-sm-3" style="flex:5;">
		<table class="tableDesign" >
			<tr style="font-size: 14px; font-family: 'Arial', serif;color: white; font-weight: bold; text-align: center;border-color: #1192e8;height:40px;">
				<th colspan="6" style="width:40%;text-align: center;background-color:#e93d6b;border-color: #e93d6b; text-align: center;">Arrival : Biometric Enrollment/Verification/Exemption Statistics in last <%=displayHours%> hours</th> 
			</tr>
			<tr style="font-size: 14px; font-family: 'Arial', serif;color: white; font-weight: bold; text-align: center;border-color: #1192e8;height:40px;">
				...
				<th style="width:20%;text-align: center;background-color:#e93d6b;border-color: #c41645; text-align: right;">Failure&nbsp;&nbsp;</th>
				<th style="width:20%;text-align: center;background-color:#e93d6b;border-color: #c41645; text-align: right;">Misc&nbsp;&nbsp;</th>
			</tr>
<%

try
	{
	SQLQUERY_B_Arrh = "select * from (select to_date(to_char(pax_boarding_date,'dd/mm/yyyy')||':'||hours,'dd/mm/yyyy:HH24mi') as date_time, to_char(pax_boarding_date,'Mon-dd') as show_date,icp_description,hours,HOURLY_BIO_ENROLLED,HOURLY_BIO_VERIFIED,HOURLY_BIO_EXEMPTED,HOURLY_BIO_FAILURE,HOURLY_BIO_MISC, table_type from imigration.im_dashboard_combined where pax_boarding_date >= trunc(sysdate-1) and table_type='IM_TRANS_ARR_TOTAL' and icp_srno = '" + filter_icp + "' order by pax_boarding_date desc,HOURS desc ) where rownum<="+displayHours;
	...
	while (rsTemp.next()){
		...
		//Failure_Precent_Arr = 0;
			...
			weekelyFailure_Arrh = rsTemp.getInt("HOURLY_BIO_FAILURE"); 
			weekelyMisc_Arrh = rsTemp.getInt("HOURLY_BIO_MISC"); 
			...
			weekFailure_Arrh.append(weekelyFailure_Arrh +",");
			weekMisc_Arrh.append(weekelyMisc_Arrh +",");
			...
			Failure_sum_hunder_Arrh = weekelyFailure_Arrh * 100;
			Misc_sum_hunder_Arrh = weekelyMisc_Arrh * 100;
			Bio_Total_sum_number_Arrh = weekelyEnroll_Arrh + weekelyVerified_Arrh + weekelyExempted_Arrh + weekelyFailure_Arrh + weekelyMisc_Arrh;
			...
			Failure_Precent_Arrh = (double) Failure_sum_hunder_Arrh / Bio_Total_sum_number_Arrh;
			Misc_Precent_Arrh = (double) Misc_sum_hunder_Arrh / Bio_Total_sum_number_Arrh;
			...
			B_Failure_Arrh.append( Math.round(Failure_Precent_Arrh) + ",");
			B_Misc_Arrh.append( Math.round(Misc_Precent_Arrh) + ",");
			%>
			<tr style="font-size: 14px; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
				...
				<td style="background-color:#ffeeee;border-color:#c41645;width:20%; font-weight: bold; text-align: right;"><%=rsTemp.getInt("HOURLY_BIO_FAILURE") == 0 ? "&nbsp;": getIndianFormat(rsTemp.getInt("HOURLY_BIO_FAILURE"))%>&nbsp;</td>
				<td style="background-color:#ffeeee;border-color:#c41645;width:20%; font-weight: bold; text-align: right;"><%=rsTemp.getInt("HOURLY_BIO_MISC") == 0 ? "&nbsp;": getIndianFormat(rsTemp.getInt("HOURLY_BIO_MISC"))%>&nbsp;</td>
			</tr>
			<%
	}
			rsTemp.close();
	...
	//out.println(strweekDaysBio_Arrh + "<br>");
			...
			strweekFailure_Arrh = B_Failure_Arrh.toString();
			strweekFailure_Arrh = strweekFailure_Arrh.substring(0,strweekFailure_Arrh.length()-1);

			strweekMisc_Arrh = B_Misc_Arrh.toString();
			strweekMisc_Arrh = strweekMisc_Arrh.substring(0,strweekMisc_Arrh.length()-1);
	}
	catch(Exception e)
	{
		e.printStackTrace();
		out.println("Arrival : Biometric Enrollment/Verification/Exemption Statistics in hours");
	}
			%>
		</table>
		</div>
	...
	try {
		...
		while (rsTemp.next()) {
			...
			<tr style="font-size: 14px; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
				...
				<td style="background-color:#ffeeee;border-color:#A4133C; font-weight: bold; text-align: right; font-size:15px; color:#e72a5c;"><%=arr_Daywise.get(weeklyGenderXAxis_Arrg)%>&nbsp;-&nbsp;<%=(rsTemp.getInt("sum_hourly_male_count") + rsTemp.getInt("sum_hourly_female_count") + rsTemp.getInt("sum_hourly_others_count"))%><BR>=&nbsp;<%=Integer.parseInt(arr_Daywise.get(weeklyGenderXAxis_Arrg)) - (rsTemp.getInt("sum_hourly_male_count") + rsTemp.getInt("sum_hourly_female_count") + rsTemp.getInt("sum_hourly_others_count"))%></td>
			</tr>
			<%
		}
		rsTemp.close();
	...
	try {
		...
		while (rsTemp.next()) {
			...
			<tr style="font-size: 14px; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
				...
				<td style="background-color:#f3faff;border-color:#014F86; font-weight: bold; text-align: right; font-size:15px; color:#0273c4;"><%=dep_Daywise.get(weeklyGenderXAxis_Depg)%>&nbsp;-&nbsp;<%=(rsTemp.getInt("sum_hourly_male_count") + rsTemp.getInt("sum_hourly_female_count") + rsTemp.getInt("sum_hourly_others_count"))%><BR>=&nbsp;<%=Integer.parseInt(dep_Daywise.get(weeklyGenderXAxis_Depg)) - (rsTemp.getInt("sum_hourly_male_count") + rsTemp.getInt("sum_hourly_female_count") + rsTemp.getInt("sum_hourly_others_count"))%></td>
...
try
  {
%>
<form action="im_icp_dashboard_00_test_validation.jsp#Arr_Sch_Flts" method="post" >
...
<table class="tableDesign">
	...
	<tr style="background-color:#a56eff;font-weight:bold;font-size: 12px;height: 30px;">
		<th style="text-align:left;background-color:#a56eff; border-color: #a56eff; color: white;font-weight:normal;" colspan="6">List of flights in last <select type="text" name="filter_time" style="border-radius: 3px;">
		...
		<th style="text-align:right;background-color:#a56eff; border-color: #a56eff; color: white; font-weight:normal;" colspan="5">Current Server Time : <%=vFileName%></th>
	</tr>
		...
		if(filter_icp.trim().length() > 0) str_filter_icp = " and icp_srno = '"+filter_icp+"'";
	        ...
	        if(calTime > 0)
				ps = con.prepareStatement("select DISTINCT ICP_SRNO,FLIGHT_NO,SCHEDULE_TIME from imigration.IM_DASHBOARD_SCH_FLIGHTS_ARR where SCHEDULE_DATE >= trunc(sysdate-2) and SCHEDULE_TIME >= '"+filter_time+"' "+str_filter_icp+" order by SCHEDULE_TIME,FLIGHT_NO");
			else
				ps = con.prepareStatement("select DISTINCT ICP_SRNO,FLIGHT_NO,SCHEDULE_TIME from imigration.IM_DASHBOARD_SCH_FLIGHTS_ARR where SCHEDULE_DATE >= trunc(sysdate-2) and SCHEDULE_TIME >= '0000' "+str_filter_icp+" order by SCHEDULE_TIME,FLIGHT_NO");
			...
			while(rs.next())
			 {	
				biometric_count = 0;
				 if(calTime > 0)
					sql_qry = "select ICP_SRNO, FLIGHT_NO, SCHEDULE_DATE, SCHEDULE_TIME, APIS_FILENAME, APIS_RECEIVED_TIME, PROCESS_START_TIME, PROCESS_END_TIME, APIS_COUNT, BIOMETRIC_COUNT, EVISA_COUNT,APIS_EVISA_COUNT, PAX_COUNT,STATUS_COLOR,TIME_DIFFERENCE,PROCESS_TIME_DIFFERENCE,FIRST_PAX_TIME,LAST_PAX_TIME,PAX_CLEARED_95,FLIGHT_CLEARED_TIME from imigration.IM_DASHBOARD_SCH_FLIGHTS_ARR where SCHEDULE_DATE >= to_date('"+filter_date+"','dd/mm/yyyy') and SCHEDULE_TIME >= '"+filter_time+"' and SCHEDULE_DATE <= trunc(sysdate) "+str_filter_icp+" and ICP_SRNO = '"+rs.getString("ICP_SRNO")+"' and FLIGHT_NO = '"+rs.getString("FLIGHT_NO")+"' order by SCHEDULE_DATE,SCHEDULE_TIME";
				else
					...
					{
						////////////////////////////////////
						ps_in = con.prepareStatement("select BIOMETRIC_NO from imigration.IM_DASHBOARD_APIS_BIOMETRIC_TEMP where FLIGHT_SCH_ARR_DATE = to_date('"+sdf.format(rs1.getDate("SCHEDULE_DATE"))+"','dd/mm/yyyy') and FLIGHT_NO = '"+rs1.getString("FLIGHT_NO")+"'");
						...
						rs1.close();
						ps1.close();
						ps1 = con.prepareStatement("select * from imigration.IM_DASHBOARD_SCH_FLIGHTS_ARR where ICP_SRNO = '"+rs.getString("ICP_SRNO")+"' and FLIGHT_NO = '"+rs.getString("FLIGHT_NO")+"' order by SCHEDULE_DATE desc");
						rs1 = ps1.executeQuery();
						if(rs1.next())
						{
							////////////////////////////////////
...
<table class="tableDesign">
	<thead>
		...
		if(filter_icp.trim().length() > 0) str_filter_icp = " and icp_srno = '"+filter_icp+"'";
	        ...
	        if(calTime > 0)
				ps = con.prepareStatement("select DISTINCT ICP_SRNO,FLIGHT_NO,SCHEDULE_TIME from imigration.IM_DASHBOARD_SCH_FLIGHTS_DEP where SCHEDULE_DATE >= trunc(sysdate-2) and SCHEDULE_TIME >= '"+filter_time+"' "+str_filter_icp+" order by SCHEDULE_TIME,FLIGHT_NO");
			else
				ps = con.prepareStatement("select DISTINCT ICP_SRNO,FLIGHT_NO,SCHEDULE_TIME from imigration.IM_DASHBOARD_SCH_FLIGHTS_DEP where SCHEDULE_DATE >= trunc(sysdate-2) and SCHEDULE_TIME >= '0000' "+str_filter_icp+" order by SCHEDULE_TIME,FLIGHT_NO");
			...
			while(rs.next())
			{
				 ...
				 if(calTime > 0)
					sql_qry_Dep = "select ICP_SRNO, FLIGHT_NO, SCHEDULE_DATE, SCHEDULE_TIME, APIS_FILENAME, APIS_RECEIVED_TIME, PROCESS_START_TIME, PROCESS_END_TIME, APIS_COUNT, BIOMETRIC_COUNT, EVISA_COUNT, PAX_COUNT,STATUS_COLOR,TIME_DIFFERENCE,PROCESS_TIME_DIFFERENCE,FIRST_PAX_TIME,LAST_PAX_TIME,PAX_CLEARED_95,FLIGHT_CLEARED_TIME from imigration.IM_DASHBOARD_SCH_FLIGHTS_DEP where SCHEDULE_DATE >= to_date('"+filter_date+"','dd/mm/yyyy') and SCHEDULE_TIME >= '"+filter_time+"' and SCHEDULE_DATE <= trunc(sysdate) "+str_filter_icp+" and ICP_SRNO = '"+rs.getString("ICP_SRNO")+"' and FLIGHT_NO = '"+rs.getString("FLIGHT_NO")+"' order by SCHEDULE_DATE,SCHEDULE_TIME";
			   ...
			   //out.println(sql_qry_Dep);
					...
					ps1.close();
						...
						rs1.close();
						ps1.close();
						ps1 = con.prepareStatement("select * from imigration.IM_DASHBOARD_SCH_FLIGHTS_DEP where ICP_SRNO = '"+rs.getString("ICP_SRNO")+"' and FLIGHT_NO = '"+rs.getString("FLIGHT_NO")+"' order by SCHEDULE_DATE desc");
						rs1 = ps1.executeQuery();
						if(rs1.next())
						{
							%>
