...
function letternumber(e, str)
		...
		function compare_report()
		{
				...
				document.entryfrm.action="im_icp_dashboard_00_test_ip_main_01.jsp?&icp="+document.entryfrm.compare_icp.value;
		...
		{
				document.entryfrm.target="_self";
				document.entryfrm.action="im_icp_dashboard_00_test_ip_main_01.jsp?&icp="+document.entryfrm.compare_ip.value;
				//document.entryfrm.action="test2.jsp?&icp="+document.entryfrm.compare_icp.value;
				document.entryfrm.submit();
				return true;
		}
		  ...
		  <input class="input" type="hidden" name="ReverseCounterID" size="55" maxlength="55" value="600">
			<table align="center" width="80%" cellspacing="0"  cellpadding="4" border="0">
				<tr bgcolor="#e8ecf1">
					...
					<font face="Verdana" color="#347FAA" size="2"><b>&nbsp;&nbsp;&nbsp;&nbsp;Requester's Name</b>
					...
					<font face="Verdana" color="#347FAA" size="2"><b>&nbsp;&nbsp;&nbsp;&nbsp;Requester's Location</b>
					<input name="r_location" id ="ip" name height="40"type="text" style="color:black;font-weight:bold; height: 28px;  width:137px; background-color: white; font-size=12pt;text-transform:uppercase;font-family:Verdana" size="4" maxlength="15"  onkeyup="filtery(this.value,this.form.compare_ip)" onchange="filtery(this.value,this.form.compare_ip)" onKeyDown="if(event.keyCode==13) event.keyCode=9;if (event.keyCode==8) event.keyCode=37+46;" onKeyPress="return letternumber(event, 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890, ')">
...
<%
					...
					{
%>
						<option value="<%=rsTemp.getString("ICP_SRNO")%>" <%if(filter_icp.equals(rsTemp.getString("ICP_SRNO"))){%> selected<%}%>><%=capitalizeFirstChar(rsTemp.getString("ICP_DESC"))%></option>
		<%
					}
					 rsTemp.close();  
					  div_hgt = 200; 
...
%>	<tr style="font-size: 14px; font-family: 'Arial', serif; text-align: center; border-color: #6929c4;height:18px;"> 
		<td style="background-color:#f4fbff; border-color: #004076; font-weight: bold;text-align: center;color: #044e8d;"><%=srno%></td>
		<td style="background-color:#f4fbff; border-color: #004076; font-weight: bold;text-align: center;color: #044e8d;"><%=ip_Address == null ? "" : ip_Address%></td>
		<td style="background-color:#f4fbff; border-color: #004076; font-weight: bold;text-align: center;color: #044e8d;"><%=icp_Number == null ? "" : icp_Number.replace("#",", ")%></td>
 		<td style="background-color:#f4fbff; border-color: #004076; font-weight: bold;text-align: center;color: #044e8d;"><%=ICP_DESCRING.substring(1,ICP_DESCRING.length())%></td>
 		<td style="background-color:#f4fbff; border-color: #004076; font-weight: bold;text-align: center;color: #044e8d;"><%=print_Requesters_Name == null ? "" : capitalizeFirstChar(print_Requesters_Name)%></td>
		<td style="background-color:#f4fbff; border-color: #004076; font-weight: bold;text-align: center;color: #044e8d;"><%=print_Requesters_Location == null ? "" : capitalizeFirstChar(print_Requesters_Location)%></td>
	 </tr>
<%			
		}
		rsTemp.close();
