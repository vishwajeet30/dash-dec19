...
try {
	...
	String strSql5 = "SELECT ICP_SRNO, ICP_DESC FROM IM_ICP_LIST where icp_srno in (select icp_srno from IM_DASHBOARD_EPASSPORT_ICP) order by ICP_DESC";
	try
	{
...
//=================================================
		...
		if(filter_icp.equals("All")) {dash = "All ICPs";  div_hgt = 600;}
			...
			rsTemp = st_icp.executeQuery("SELECT ICP_SRNO, ICP_DESC FROM IM_ICP_LIST where icp_srno in (select icp_srno from IM_DASHBOARD_EPASSPORT_ICP) order by ICP_DESC");
			while(rsTemp.next())
			{
...
function compare_report()
{
	...
	document.entryfrm.action="im_epassport_dashboard.jsp?&icp="+document.entryfrm.compare_icp.value;
		...
		<div class="container"> 
		  ...
		  <div class="collapse navbar-collapse" id="collapsibleNavbar">
			<ul class="navbar-nav">
			  ...
			  <li> <a class="scrollLink dropdown-item" href="#ICP_3">Country-wise Arrival and Departure e-Passport Statistics</a></ul></li>
...
<%
	...
	try {
		dashQuery = "select count(1) as arr_epassport from IM_TRANS_ARR_TOTAL where PAX_BOARDING_DATE = trunc(sysdate-1) and EPASSPORT_FLAG = 'Y' " + vishwaFilter.replace("where","and") ;
		psTemp = con.prepareStatement(dashQuery);
		rsTemp = psTemp.executeQuery();
		if (rsTemp.next()) {
	...
	try {
		dashQuery = "select count(1) as arr_epassport from IM_TRANS_ARR_TOTAL where PAX_BOARDING_DATE = trunc(sysdate) and EPASSPORT_FLAG = 'Y' " + vishwaFilter.replace("where","and") ;
		psTemp = con.prepareStatement(dashQuery);
		rsTemp = psTemp.executeQuery();
		if (rsTemp.next()) {
...
try {
	dashQuery = "select count(1) as dep_Passenger_Count from IM_TRANS_DEP_TOTAL where PAX_BOARDING_DATE = trunc(sysdate-1) and EPASSPORT_FLAG = 'Y' " + vishwaFilter.replace("where","and") ;
	psTemp = con.prepareStatement(dashQuery);
	rsTemp = psTemp.executeQuery();
	if (rsTemp.next()) {
...
try {
	dashQuery = "select count(1) as dep_Passenger_Count from IM_TRANS_DEP_TOTAL where PAX_BOARDING_DATE = trunc(sysdate) and EPASSPORT_FLAG = 'Y' " + vishwaFilter.replace("where","and") ;
	psTemp = con.prepareStatement(dashQuery);
	rsTemp = psTemp.executeQuery();
	if (rsTemp.next()) {
...
try
	{
	SQLQUERY = "";
	SQLQUERY = "select year as arr_year,sum(ARRIVAL_PAX) as arr_count,sum(DEPARTURE_PAX) as dep_count from IM_DASHBOARD_EPASSPORT_ICP "+ vishwaFilter +" group by year order by 1 desc";
...
<div class="card"style="border: solid 3px #007d79; border-radius: 20px;height:900px;">
				...
				<td>
					<div class="col-sm-2" style="width:450px;">
						...
						<tr style="font-size: 15px;  text-align: right; color:white; border-color: #c10065;height:12px;background-color:#ed459d;">
							...
							<th style="background-color:#ed459d;border-color: #c10065; text-align: right;width:22%;">Arrival&nbsp;</th>
							<th style="background-color:#ed459d;border-color: #c10065; text-align: right;width:23%;">Departure&nbsp;</th>
							<th style="background-color:#ed459d;border-color: #c10065; text-align: right;width:25%;">Total&nbsp;</th>
						</tr>
<% 
					}
...
%>						<tr style="font-size: 15px;  text-align: right; color:white; border-color: #c10065;height:12px;background-color:#ed459d;">
						...
						<td style="background-color:#ed459d;border-color:#c10065; font-weight: bold; text-align: right;width:23%;"><%=getIndianFormat(ePass_Dep_Total)%>&nbsp;</td>
						<td style="background-color:#ed459d;border-color:#c10065; font-weight: bold; text-align: right;color:white;font-size: 15px;width:25%;"><%=getIndianFormat(ePass_Total)%>&nbsp;</td>
					</tr>
<%					} 
					else 
					{ 
		...
		var myoptionsp = {
			...
			onComplete: function () {
				var chartInstances = this.chart,
					...
					ctx.fillStyle = "#0F292F";
					ctx.textBaseline = 'bottom';
					ctx.font = "bold 10.5px Verdana";

					this.data.datasets.forEach(function (dataset, i) {
						var metas = chartInstances.controller.getDatasetMeta(i);
						metas.data.forEach(function (bar, index) {
							var data = dataset.data[index];
							ctx.fillText(data.toLocaleString('en-IN'), bar._model.x+28, bar._model.y+6);
...
try
	...
	while (rsTemp.next()) {
			...
			<tr style="font-size: 15px;  color:white; border-color: #3B7DD8;height:12px;border-radius: 20px;">
				...
				<th style="background-color:#6D5EB4;border-color: #382497; text-align: left;width:40%;">Country&nbsp;</th>
				<th style="background-color:#6D5EB4;border-color: #382497; text-align: right;width:15%;">Arrival&nbsp;</th>
				<th style="background-color:#6D5EB4;border-color: #382497; text-align: right;width:15%;">Departure&nbsp;</th>
				<th style="background-color:#6D5EB4;border-color: #382497; text-align: right;width:20%;">Total&nbsp;</th>
			</tr>
			<%
			}
			%>
			<tr style="font-size: 14px;color:black; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
				...
				<td style="background-color:#efedf6;border-color:#776ab9;width:40%; font-weight: bold;text-align: left;"><%=naltyValue.get(rsTemp.getString("NALTY_CODE")) == null ? rsTemp.getString("NALTY_CODE") : naltyValue.get(rsTemp.getString("NALTY_CODE")).replace(" ", "&nbsp;").replace("-", "&#8209;")%>&nbsp;</td>
				<td style="background-color:#efedf6;border-color:#776ab9;width:15%; font-weight: bold; text-align: right;"><%=rsTemp.getInt("arr_count") == 0 ? "&nbsp;" : getIndianFormat(rsTemp.getInt("arr_count"))%>&nbsp;</td>
				<td style="background-color:#efedf6;border-color:#776ab9;width:15%; font-weight: bold; text-align: right;"><%=rsTemp.getInt("dep_count") == 0 ? "&nbsp;" : getIndianFormat(rsTemp.getInt("dep_count"))%>&nbsp;</td>
				<td style="background-color:#efedf6;border-color:#776ab9;width:20%; font-weight: bold; text-align: right;color:#554799;font-size: 15px;"><%=getIndianFormat((rsTemp.getInt("arr_count") + rsTemp.getInt("dep_count")))%>&nbsp;</td>
			</tr>
<%
				/*country_Arr_Total = country_Arr_Total + rsTemp.getInt("arr_count");
				country_Dep_Total = country_Dep_Total + rsTemp.getInt("dep_count");
...
<div class="card" style="border: solid 3px #776ab9; border-radius: 20px; height:1500px;">
<div class="card-body" style="height:1500px;">
	...
	var myData_NALTY = {
		...
		var myoptions = {
				 ...
				 scales: {
					 ...
					 title: {
							display: true,
								text: 'Top 20 : Country-wise Arrival and Departure e-Passport Statistics',
								fontSize:25,
							},
			tooltips: {
				enabled: true
			...
			onComplete: function () {
				var chartInstances = this.chart,
					...
					this.data.datasets.forEach(function (dataset, i) {
						...
						metas.data.forEach(function (bar, index) {
							var data = dataset.data[index];
							ctx.fillText(data.toLocaleString('en-IN'), bar._model.x+40, bar._model.y+6);
...
counterAnim("#countArrT", 50, <%=getIndianFormat(today_Arrival_Count)%>, 2200);
...
counterAnim("#count_total_Dep_CountT", 50, <%=getIndianFormat(today_Dep_Count)%>, 2200);
