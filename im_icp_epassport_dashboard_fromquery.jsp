...
try
    ...
    int srno = 0;
	SQLQUERY = "";
	SQLQUERY = "select DG01_NATLY_CODE,count(1) as arr_count from IM_TRANS_ARR_EPASSPORT group by DG01_NATLY_CODE order by 2 desc";
	
	psTemp = con.prepareStatement(SQLQUERY);
	rsTemp = psTemp.executeQuery();
	while (rsTemp.next()) {
		SQLQUERY = "";
		SQLQUERY = "select count(1) as dep_count from IM_TRANS_DEP_EPASSPORT where DG01_NATLY_CODE = '"+rsTemp.getString("DG01_NATLY_CODE")+"'";
		...
		if (rsMain.next()) {
			...
			{
			NaltyPax.append("\"");
			NaltyPax.append(rsTemp.getString("DG01_NATLY_CODE"));
			NaltyPax.append("\"");
			NaltyPax.append(",");
			ArrNaltyPax.append(rsTemp.getInt("arr_count")+",");
			DepNaltyPax.append(rsMain.getInt("dep_count")+",");
			}
			%>
			<tr style="font-size: 14px;color:white; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
				<td style="background-color:#4A91F2;border-color:#4A91F2;width:33.33%; font-weight: bold;text-align: center;"><%=++srno%></td>
				<td style="background-color:#4A91F2;border-color:#4A91F2;width:33.33%; font-weight: bold;text-align: center;"><%=rsTemp.getString("DG01_NATLY_CODE")%></td>
				<td style="background-color:#64A1F4;border-color:#64A1F4;width:33.33%; font-weight: bold; text-align: right;"><%=rsTemp.getInt("arr_count")%>&nbsp;</td>
				<td style="background-color:#8DBDFF;border-color:#8DBDFF;width:33.33%; font-weight: bold; text-align: right;"><%=rsMain.getInt("dep_count")%>&nbsp;</td>
