...
<%!
public String getIcpDesc(String nalty,Connection con)
{
	String desc = "";
	PreparedStatement ps = null;
	ResultSet rs = null;
	try {
		ps = con.prepareStatement("select NALTY_DESC from im_country where NALTY_CODE = '"+nalty+"'");
		rs = ps.executeQuery();
		if(rs.next())
			desc = rs.getString("NALTY_DESC");
		rs.close();
		ps.close();
...
<body onload="DigitalTime(); StartTimer();">
	<div class="wrapper">
	<SPAN id="Clock" style="position:absolute;top:11px;width:250px;height:500px;" ></span>
		...
		<div class="container"> 
		  ...
		  <div class="collapse navbar-collapse" id="collapsibleNavbar">
			<ul class="navbar-nav">
			  ...
			  <li> <a class="scrollLink dropdown-item" href="#ICP_3">Country-wise Arrival and Departure e-Visa Statistics</a></li>
			  <li> <a class="scrollLink dropdown-item" href="#ICP_4">Country-wise Arrival and Departure e-Visa Statistics</a></ul></li>
...
%>
	...
	<table id = "auto-index1" class="table table-sm table-striped" >
			<thead>
				...
				<%} else {%>
					<th colspan=4 style="font-family: Arial;background-color: #1192e8; color: white; font-size: 20px;text-align: left;"><%=capitalizeFirstChar(dash.replace("INTERNATIONAL",""))%> : Arrival e-Passport Statistics</th>
				<%} %>
				</tr>
...
try
	{
	SQLQUERY = "";
	SQLQUERY = "select year as arr_year,sum(EVISA_PAX) as arr_count,sum(EVISA_PAX) as dep_count from IM_DASHBOARD_EVISA_ICP "+ vishwaFilter +" group by year order by 1";
...
%>						<tr style="font-size: 14px;color:black; font-family: 'Arial', serif; border-color: #ed459d;height:18px;">
						...
						<td style="background-color:#fdeef6;border-color:#f6a2ce; font-weight: bold;text-align: left;width:20%;"><%=icpValue.get(rsTemp.getString("ICP_SRNO")) == null ? rsTemp.getString("ICP_SRNO"):icpValue.get(rsTemp.getString("ICP_SRNO")) %></td>
		...
		var myoptionsp = {
			maintainAspectRatio: false,
				 scales: {
					  ...
					  title: {
							display: true,
								text: 'Top 20 : ICP-wise Arrival e-Passport Statistics',
							fontSize: 25,
...
try
	...
	while (rsTemp.next()) {
			...
			<tr style="font-size: 14px;color:black; font-family: 'Arial', serif; border-color: #6929c4;height:18px;">
				...
				<td style="background-color:#efedf6;border-color:#776ab9;width:15%; font-weight: bold;text-align: center;"><div class="tooltip1"><%=rsTemp.getString("NALTY_CODE")%><span class="tooltiptext1"><%=getIcpDesc(rsTemp.getString("NALTY_CODE"),con)%></span></div></td>
	...
	var myData_NALTY = {
		...
		var myoptions = {
				 ...
				 scales: {
					 ...
					 title: {
							display: true,
								text: 'Country-wise Arrival e-Visa Statistics',
								fontSize:25,
							},
			tooltips: {
				enabled: true
