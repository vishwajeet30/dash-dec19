...
<div id="loading" name="loading/">
	<br><br>
	<br><br><CENTER><font face="verdana" color="blue" size="3"><b>Flights Scheduling Arrival Trans Time
...
<BR><BR>
<form action="im_dashboard_schedule_flights_arr_final_nov13.jsp" method="post">
...
<td>Current Time <%=vFileName%>, Time Before <%=select_time%> Hrs : <%=filter_date %>&nbsp;<%=filter_time.substring(0,2)+":"+ filter_time.substring(2,4)%></td>
</tr>
</table>-->
<%
...
%>
<table align="center" border="1" id="theTable" width="60%" cellpadding="8" cellspacing="0" bordercolorlight="#FF99CC" bordercolordark="#FF99CC" bordercolor="#FF99CC" style="border-collapse: collapse;background-color:#FFFFFF;font-family:verdana;font-size:8pt;text-align:right;">
			<tr style="background-color:#E6E6EA;font-weight:bold">
				<td style="text-align:left;" colspan="7">List of flights since <%=filter_date %>&nbsp;<%=filter_time.substring(0,2)+":"+ filter_time.substring(2,4)%></td>
				<td style="text-align:right;" colspan="11">Server Time : <%=vFileName%></td>
			   ...
			   //out.println(sql_qry);
					...
					if(rs1.next())
					{
						...
						<tr> 
							...
							<td style="text-align:left;color:#9DCEFF"><%=sdf.format(rs1.getDate("SCHEDULE_DATE"))%>&nbsp;<font color='black'><%=rs1.getString("SCHEDULE_TIME").substring(0,2)+":"+rs1.getString("SCHEDULE_TIME").substring(2,rs1.getString("SCHEDULE_TIME").trim().length())%></font></td>
